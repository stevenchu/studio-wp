...
///////////////////////////////
function introInit() {
	/*custom steven chu for intro*/
	jQuery("#intro").click(function(e){
		e.preventDefault();
...
function showProject(projectSlug) {		
	...
	jQuery("#projectHolder .closeBtn").click(function(){
		...
		jQuery(".thumbs .selected").find('img:last').stop().fadeTo("fast", 1);	
		jQuery(".thumbs .selected").removeClass("selected");		
		jQuery.scrollTo(jQuery('div#content'), scrollSpeed);	
		processProject();
	});
}
...
function gridResize() {	
	...
	var container = jQuery('.thumbs.masonry');
	var colW = container.width() * 0.32;
	var colG = container.width() * 0.01;
